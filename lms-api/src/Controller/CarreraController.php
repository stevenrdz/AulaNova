...
use App\DTO\CarreraCreateRequest;
use App\DTO\CarreraUpdateRequest;
...
class CarreraController extends ApiController
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $qb = $this->entityManager->getRepository(Carrera::class)->createQueryBuilder('c');
        ...
        return $this->json([
            'data' => array_map(fn (Carrera $item) => $this->mapItem($item), $items),
            'meta' => [
                'page' => $page,
                'limit' => $limit,
                'total' => $total,
    ...
    {
        $item = $this->entityManager->getRepository(Carrera::class)->find($id);
        if (!$item) {
            return $this->json(['message' => 'Registro no encontrado.'], 404);
        }

        $data = json_decode($request->getContent(), true) ?? [];
        $dto = new CarreraUpdateRequest();
    ...
    private function mapItem(Carrera $item): array
    {
        return [
            'id' => $item->getId(),
            'name' => $item->getName(),
