...
use App\DTO\AsignaturaCreateRequest;
use App\DTO\AsignaturaUpdateRequest;
...
class AsignaturaController extends ApiController
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $qb = $this->entityManager->getRepository(Asignatura::class)->createQueryBuilder('a');

        if ($q !== '') {
            $qb->andWhere('LOWER(a.name) LIKE :q')
        ...
        return $this->json([
            'data' => array_map(fn (Asignatura $item) => $this->mapItem($item), $items),
            'meta' => [
                'page' => $page,
                'limit' => $limit,
                'total' => $total,
    ...
    public function create(Request $request, ValidatorInterface $validator): JsonResponse
    {
        ...
        $dto = new AsignaturaCreateRequest();
        $dto->name = $data['name'] ?? '';
    ...
    {
        $item = $this->entityManager->getRepository(Asignatura::class)->find($id);
        if (!$item) {
            return $this->json(['message' => 'Registro no encontrado.'], 404);
        }

        $data = json_decode($request->getContent(), true) ?? [];
        $dto = new AsignaturaUpdateRequest();
    ...
    private function mapItem(Asignatura $item): array
    {
        return [
            'id' => $item->getId(),
            'name' => $item->getName(),
