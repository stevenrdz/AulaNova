...
use App\DTO\NivelCreateRequest;
use App\DTO\NivelUpdateRequest;
...
#[Route('/structure/niveles')]
#[IsGranted('ROLE_ADMIN')]
class NivelController extends ApiController
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $qb = $this->entityManager->getRepository(Nivel::class)->createQueryBuilder('n');
        ...
        return $this->json([
            'data' => array_map(fn (Nivel $item) => $this->mapItem($item), $items),
            'meta' => [
                'page' => $page,
                'limit' => $limit,
                'total' => $total,
    ...
    {
        $item = $this->entityManager->getRepository(Nivel::class)->find($id);
        if (!$item) {
            return $this->json(['message' => 'Registro no encontrado.'], 404);
        }

        $data = json_decode($request->getContent(), true) ?? [];
        $dto = new NivelUpdateRequest();
    ...
    private function mapItem(Nivel $item): array
    {
        return [
            'id' => $item->getId(),
            'name' => $item->getName(),
