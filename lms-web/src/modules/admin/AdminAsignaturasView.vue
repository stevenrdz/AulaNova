<template>
  <PageHeader
    title="Asignaturas"
    subtitle="Gestion de asignaturas."
  />
  <BaseCard>
...
const modalTitle = computed(() => (editingId.value ? 'Editar asignatura' : 'Nueva asignatura'))
...
const load = async () => {
  ...
  try {
    ...
    const { data } = await structureApi.asignaturas.list(params)
...
const save = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await structureApi.asignaturas.update(editingId.value, payload)
    } else {
      await structureApi.asignaturas.create(payload)
    }
...
const remove = async (item: Item) => {
  ...
  try {
    await structureApi.asignaturas.remove(item.id)
    await load()
  } catch (err: any) {
