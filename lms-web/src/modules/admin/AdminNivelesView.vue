...
const modalTitle = computed(() => (editingId.value ? 'Editar nivel' : 'Nuevo nivel'))
...
const load = async () => {
  ...
  try {
    ...
    const { data } = await structureApi.niveles.list(params)
...
const save = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await structureApi.niveles.update(editingId.value, payload)
    } else {
      await structureApi.niveles.create(payload)
    }
...
const remove = async (item: Item) => {
  ...
  try {
    await structureApi.niveles.remove(item.id)
    await load()
  } catch (err: any) {
